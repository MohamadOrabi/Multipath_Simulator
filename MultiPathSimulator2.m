...
%     end
    ...
    if (~NNErrorFlag)
        SamplesData(n,:) = R(round(code_shift_hat_DLL(n))-delta_shift_samples+1:round(code_shift_hat_DLL(n))+delta_shift_samples);
        SamplesData_noiseless(n,:) = R_noiseless(round(code_shift_hat_DLL(n))-delta_shift_samples+1:round(code_shift_hat_DLL(n))+delta_shift_samples);
    else
...
try
    ...
    warning('code_shift_hat did not plot')
end
