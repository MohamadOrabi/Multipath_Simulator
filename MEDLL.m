...
code191 = resample_PRN1(code19,samplesPerCode,chip_rate,fs,0);
...
ShiftsData = zeros(runs,1); %Errros from the center of the MDL
...
code_shift_hat_MEDLL(1) = round(shift_center_hi + mod(pseudorange(sat,1),300e3)/c*fs); % Initial Value 
...
for n = 1:runs
    ...
    %y_lo_noiseless = code19_d_lo + code19_Multipath_lo;% + noise;% + code25_d + code5_d;
    y = code19_d + code19_Multipath;
    
    %Resampling to low fs
    code19_d_lo = decimate(code19_d,f_ratio);
...
MEDLL_RMSE = norm(MEDLLError)/sqrt(runs)/fs*c
